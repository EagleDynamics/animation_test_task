...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE /*hPrevInstance*/, LPSTR /*lpCmdLine*/, int /*nShowCmd*/)
{
	...
	{
		const char* file_name = "data/model.fbx";
		Assimp::Importer importer;
		importer.SetPropertyBool(AI_CONFIG_IMPORT_FBX_PRESERVE_PIVOTS, false);
		const aiScene* scene = importer.ReadFile(file_name,	aiProcess_CalcTangentSpace | aiProcess_Triangulate | aiProcess_JoinIdenticalVertices | aiProcess_SortByPType);
		if (nullptr == scene) 
		{
...

		{
			Assimp::Importer importer;
			importer.SetPropertyBool(AI_CONFIG_IMPORT_FBX_PRESERVE_PIVOTS, false);
			const aiScene* anim_scene = importer.ReadFile("data/walk.fbx", 0);
			if (nullptr == anim_scene)
			{
				std::cout << "error reading anim_scene\n";
				return false;
			}
			aiNode* root = anim_scene->mRootNode;
			print_node(root);
		}
...

		{
			...
			vertexBufferDesc.ByteWidth = mesh->mNumVertices * sizeof(mesh->mVertices[0]);
			vertexBufferDesc.Usage = D3D11_USAGE_IMMUTABLE;
			vertexBufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;

			D3D11_SUBRESOURCE_DATA vertexSubresourceData = { mesh->mVertices };
		...
		{
			D3D11_BUFFER_DESC vertexBufferDesc = {};
			vertexBufferDesc.ByteWidth = mesh->mNumVertices * sizeof(mesh->mNormals[0]);
			vertexBufferDesc.Usage = D3D11_USAGE_IMMUTABLE;
			vertexBufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;

			D3D11_SUBRESOURCE_DATA vertexSubresourceData = { mesh->mNormals };

			HRESULT hResult = d3d11Device->CreateBuffer(&vertexBufferDesc, &vertexSubresourceData, &normalBuffer);
			assert(SUCCEEDED(hResult));
		}
		{
			D3D11_BUFFER_DESC indexBufferDesc = {};
			indexBufferDesc.ByteWidth = mesh->mNumFaces * sizeof(uint32_t)*3;
			...
			D3D11_SUBRESOURCE_DATA indexSubresourceData = { &indices[0]};
	...
	while (isRunning)
	{
		...
		// Update camera
		{
			...
			const float CAM_MOVE_SPEED = 50.f; // in metres per second
		...
		ID3D11Buffer* pVertexBuffers[] = { vertexBuffer, normalBuffer };
		UINT stride[] = {sizeof(float)*3, sizeof(float) * 3 };
		UINT offset[] = {0, 0};
		d3d11DeviceContext->IASetVertexBuffers(0, 2, pVertexBuffers, stride, offset);
